...
const Game = props => {
  ...
  return (
    <div className={"gameContainer"}>
      {currentGame ? (
        <>
          {currentGame.items && currentGame.items.item.name.value ? (
            <div>
              <img
                src={currentGame.items.item.thumbnail}
                alt={currentGame.items.item.name.value}
              />
              <h3>
                {currentGame.items.item.image &&
                currentGame.items.item.image ? (
                  <a href={currentGame.items.item.image} target={"_blank"}>
            ...
            currentGame.items.item.name[0].value && (
              <div>
                <img
                  src={currentGame.items.item.thumbnail}
                  alt={currentGame.items.item.name[0].value}
                />
                <h3>
                  {currentGame.items.item.image &&
                  currentGame.items.item.image ? (
          ...
          {showDesc && (
            <p className={"description"}>
              {currentGame.items.item.description
                .replace(/&hellip;/g, "...")
                .replace(/&#10;/g, " ")
                .replace(/&rsquo;/g, "'")
                .replace(/&ndash;/g, ":")
