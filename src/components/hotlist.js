    ...
    <div className={"hotlistContainer"}>
      ...
      {hotlist.items
        ? hotlist.items.item.map((ele, index) => {
            return (
              <div key={index}>
                <div key={index} className={"hotlistItem"}>
                  <img
                ...
                {showList && cgame.id === ele.id && (
                  <NewList handleSubmit={listAndGame} game={ele} uid={uid} />
                )}
              </div>
            );
          })
      ...
      <div className={"modalContainer"} key={cgame && cgame.id}>
        {modal && <Modal lookup={cgame.id} modal={modal} setModal={setModal} />}
      </div>
    </div>
  );
};
